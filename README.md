...
* `NewPendingAdmin` - Not core to the protocol, can easily be tracked if needed
* `NewAdmin` - Not core to the protocol, can easily be tracked if needed
* `NewOracle` - Not core to the protocol, can easily be tracked if needed
* `EquityWithdrawn` - Not core to the protocol, can easily be tracked if needed
* `SetPaused` - Not core to the protocol, can easily be tracked if needed

This subgraph can be used for Compound on the mainnet, and all testnets. In order to run it for a 
...
* Mapping scripts       (moneyMarket.ts) 
...
We have provided a quick guide on how to start up the Compound-Subgraph graph node below. If these steps aren't descriptive enough, the [getting started guide](https://github.com/graphprotocol/graph-node/blob/master/docs/getting-started.md) has in depth details on running a subgraph. 

## Steps to get the Compound-Subgraph Running 
  1. Install IPFS and run `ipfs init` followed by `ipfs daemon`
  2. Install PostgreSQL and run `initdb -D .postgres` followed by `pg_ctl -D .postgres start` and `createdb compound-subgraph-mainnet` (note this db name is used in the commands below for the mainnet examples)
...
```
  ...
  --postgres-url postgresql://USERNAME:[PASSWORD]@localhost:5432/mainnet-compound-subgraph \
  --ipfs 127.0.0.1:5001 \
...
```
    cargo run -p graph-node --release --   
    --postgres-url postgresql://USERNAME:[PASSWORD]@localhost:5432/compound-rinkeby-subgraph 
    --ipfs 127.0.0.1:5001
    --ethereum-rpc rinkeby-infura:https://rinkeby.infura.io 

```
  
 7. Now deploy the compound-subgraph to The Graph Node with `yarn deploy --verbosity debug`. You should see a lot of blocks being skipped in the `graph-node` terminal, and then it will start ingesting events from the moment the contracts were uploaded to the network. 
...
Below are a few ways to show how to query the compound-subgraph for data. 
